  ...
  describe "hspec" $ do
    ...
    describe "with --rerun" $ do
      ...
      it "reuses the same seed" $ do
        ...
        r0 <- withArgs ["--seed", "6703944653392975055776623123282860478116814948455471509475886703874123"] runSpec_
        r0 `shouldContain` [
            "Falsifiable (after 45 tests): "
          , "26"
          ]
    ...
    context "with --seed" $ do
      it "uses specified seed" $ do
        r <- captureLines . ignoreExitCode . withArgs ["--seed", "6703944653392975055776623123282860478116814948455471509475886703874123"] . H.hspec $ do
            H.it "foo" $
              property (/= (26 :: Integer))
        r `shouldContain` [
            "Falsifiable (after 45 tests): "
